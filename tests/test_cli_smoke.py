...
def test_app_help_exits_zero() -> None:
    ...
    completed = subprocess.run(
        [sys.executable, "-m", "conventional_kit.cli.app", "--help"],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    assert "conventional-kit" in completed.stdout
...
def test_help_payload_is_json_shape() -> None:
    ...
    completed = subprocess.run(
        [
            ...
            "-c",
            (
                "import json; from conventional_kit.mcp.server import describe_convention;"
