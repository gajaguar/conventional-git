...
`conventional-kit create branch`. The CLI owns every deterministic rule (type
...
3. MUST build and run the command below:
   ...
   ```bash
   conventional-kit create branch \
     --type "<type>" \
...
> NOTE: If `conventional-kit` is not available, recommend the user install it
> with `uv tool install conventional-kit` or `pipx install conventional-kit`.
