...
1. **Spec core** — `conventional_kit.commit.*` and `conventional_kit.branch.*`.
...
2. **Adapters** — `conventional_kit.adapters.*` (gitlint rules, commitizen
...
3. **Front-ends** — `conventional_kit.cli.*` and `conventional_kit.mcp.*`.
